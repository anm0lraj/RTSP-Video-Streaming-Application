...
class Client:
	...
	def createWidgets(self):
		"""Build GUI."""
		# Create Setup button
		self.setup = Button(self.master,fg="#ffffff",font=("LEMON MILK", 10), bg="#2a2a2a",width=15, padx=3, pady=3,bd=0)
		self.setup["text"] = "🔽 Setup"
		self.setup["command"] = self.setupMovie
		self.setup.grid(row=1, column=0)
		
		# Create Play button		
		self.start = Button(self.master,fg="#ffffff",font=("LEMON MILK", 10), bg="#2a2a2a",width=15, padx=3, pady=3,bd=0)
		self.start["text"] = "▶ Play"
		self.start["command"] = self.playMovie
		self.start.grid(row=1, column=1)
		
		# Create Pause button			
		self.pause = Button(self.master,fg="#ffffff",font=("LEMON MILK", 10), bg="#2a2a2a",width=15, padx=3, pady=3,bd=0)
		self.pause["text"] = "⏸ Pause"
		self.pause["command"] = self.pauseMovie
		self.pause.grid(row=1, column=2)
		
		# Create Teardown button
		self.teardown = Button(self.master, fg="#ffffff",font=("LEMON MILK", 10), bg="#2a2a2a",width=15, padx=3, pady=3,bd=0)
		self.teardown["text"] = "🔼 Teardown"
		self.teardown["command"] =  self.exitClient
		self.teardown.grid(row=1, column=3)
		
		# Create a label to display the movie
		self.label = Label(self.master,bg="#000000",height=19 ,bd=0)
		self.label.grid(row=0, column=0, columnspan=4, sticky=W+E+N+S, padx=0, pady=1) 

		# Create buttons to forward/backward movie
		self.backButton = Button(self.master, fg="#ffffff",font=("LEMON MILK", 10), bg="#2a2a2a",width=15, padx=3, pady=3,bd=0)
		self.backButton["text"] = "⏪ Back"
		self.backButton["command"] = self.moveBack
		self.backButton.grid(row=2, column=1)

		self.forwardButton = Button(self.master, fg="#ffffff",font=("LEMON MILK", 10), bg="#2a2a2a",width=15, padx=3, pady=3,bd=0)
		self.forwardButton["text"] = "⏩ Forward"
		self.forwardButton["command"] = self.moveForward
		self.forwardButton.grid(row=2, column=2)

		# Create button to restart movie
		self.restartButton = self.forwardButton = Button(self.master, fg="#ffffff",font=("LEMON MILK", 10), bg="#2a2a2a",width=15, padx=3, pady=3,bd=0)
		self.restartButton["text"] = "🔁 Restart"
		self.restartButton["command"] = self.restart
		self.restartButton.grid(row=2, column=0)

		# Create label to show time of video
		self.timeString = StringVar()
		self.timeString.set("Current / Total")
		self.timeLabel = Label(self.master,fg="#ffffff",font=("LEMON MILK", 10), bg="#2a2a2a",width=15, padx=5, pady=7, textvariable=self.timeString,bd=0)
	...
	def exitClient(self):
		...
		if self.sumOfTime > 0:
			"""open text file"""
			text_file = open("D:/SASTRA/5th Sem/CN/temp/MyExtend6(Emoji+Text)(Adv)/stats.txt", "w")
			text_file.write("Total data: " + str(self.sumOfData) + " bytes \n")
			text_file.write("Total time taken: " + str(self.sumOfTime) +" seconds\n")
			rateData = float(self.sumOfData/self.sumOfTime)
			text_file.write("Video Data Rate: " + str(rateData) +" bytes/second\n")
			rateLoss = float(self.countPacket/self.frameNbr)
			text_file.write("RTP Packet Loss Rate: " + str(rateLoss))
			text_file.close()
		os.system('cmd /k python Client_App.py')
	def pauseMovie(self):
